...
def run_main_dfm() -> None:
    ...
    for forecast_length in forecasts_steps:
        ...
        end_date_dfm = (features['date'].max() -
                        relativedelta(months=3*(config.TEST_LEN - forecast_length - 1)))
        start_date_dfm = (features['date'].max() -
                          relativedelta(months=3*(config.TEST_LEN - 1 - forecast_length - 1 + config.HORIZON)))
        end_date_dfm, start_date_dfm

        train_dfm = features.filter(
            (pl.col('date') < start_date_dfm)
        )
                ...
                for avaliability in avaliability_grid:
                    for k_factors in k_factors_grid:
                        for factor_order in factor_order_grid:
                            ...
                            logger.info(f"DFM has parameters features_type={features_type}, "
                                        f"features_strategy={features_strategy}, avaliability={avaliability}")
                            logger.info(f"k_factors={k_factors}, factor_order={factor_order}")
                            dfm = DFM(
                                train_dfm=train_dfm,
                                targets=targets,
