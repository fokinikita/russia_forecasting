    ...
    for sheet_name in sheet_names:
        try:
            ...
            for i, start in enumerate(block_starts):
                ...
                block_long = block.melt(id_vars="year", value_vars=month_cols,
                                        var_name="month", value_name="value")

                # чистим от NaN и неправильных годов
                block_long = block_long.dropna(subset=["year", "value"])
                block_long = block_long[block_long["year"].apply(lambda x: str(x).isdigit())]

                # === ЧИСТИМ значения ===
                block_long["value"] = (
                    block_long["value"]
                    .astype(str)
                    .str.replace(r"[^0-9,.\-]", "", regex=True)  # убираем скобки, символы, сноски
                    .str.replace(",", ".", regex=False)          # запятые → точки
                )

                # Преобразуем в float (невалидные → NaN)
                block_long["value"] = pd.to_numeric(block_long["value"], errors="coerce")
                ...
                block_long["date"] = pd.to_datetime(
                    block_long["year"].astype(str) + "-" + block_long["month"].astype(str) + "-01"
                )
                ...
                parsed_list = pd.pivot(result_long, index="date", columns="indicator", values="value")

                parsed_list_polars = pl.DataFrame(parsed_list.reset_index()).with_columns(
