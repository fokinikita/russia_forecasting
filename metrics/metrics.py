...
class MetricsCalculator:
    ...
    def _concat_models(self):
        ...
        for target in self.target_names:
            for model_index, model_name in enumerate(self.model_names):
                single_target_pred = self.preds_list[model_index].filter(
                    pl.col('target_name') == target
                ).join(
                    self.features.select(['date', target]).rename({target: 'target_value'}), on='date'
                ).with_columns(
                    ((pl.col(f'pred_{model_name}') - pl.col('target_value')) ** 2).alias('mse')
                )

                single_target_pred_gb = single_target_pred.group_by(['horizon', 'avaliability', 'features_type']).agg(
                    (pl.col('mse').mean() ** 0.5).alias(f'rmse')
                ).sort(['horizon', 'avaliability', 'features_type']).with_columns(
    ...
    def _get_naive(self):
        ...
        test_start_date = test_end_date - relativedelta(months=3 * (config.TEST_LEN - 1))

        pred_dates = []
        naive_list = []
        for horizon in range(1, config.HORIZON + 1):
            for target in self.target_names:
                naive_one_horizon_target = targets.with_columns(
                    ...
                    pl.col('date').is_between(test_start_date, test_end_date)
                )

                naive_one_horizon_target_gb = naive_one_horizon_target.group_by(
                    'target_name', 'horizon'
                ).agg(
                    (pl.col('mse').mean() ** 0.5).alias('rmse_naive')
                )

                naive_list.append(
                    naive_one_horizon_target_gb
                )
